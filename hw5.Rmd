...
twitch_data <- read_csv("https://raw.githubusercontent.com/vaiseys/dav-course/main/Data/twitchdata-update.csv", show_col_types = FALSE)
```
...
sample(twitch_data)
  ...
  summary(twitch_data$average_viewers)
  summary(twitch_data$followers)
...
The range for both values is very large. The minimum and maximum are several orders of magnitude apart.
...
There seems to be a positive correlation between average viewers and followers which is to be expected.
...
Now it appears linearly related.
...
Yes, there is a fairly even distribution of points above and below the the model line.
...
No, the residuals are centrally distributed around 0 and evenly above and below. 
...
fit2 <- lm(log_viewers~language, twitch_data)
tidy(fit2)
...
It appears most of the values a negative therefore the prediction is holding thus far.
...
ggplot(pred_data2, aes(.resid, language)) +
  geom_point() 

filter(twitch_data, log_viewers>5)
```
