...
```{r}
bootstrap1 <- rep_sample_n(poll, 100, replace = T, 1200)

bootstrap_1_data <- bootstrap1 %>% 
  group_by(replicate) %>% 
...
```{r}
#| message: false
rap <- read_csv("https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-04-14/polls.csv")

rap <- filter(rap, rank == 1)
...
```{r}
rap_year <- select(rap, year)

rap_data <- rap_year %>%
  summarize(prop = mean(year))
...
This particular iteration's is smaller than the full samples.

```{r}
rapstrap <- sample_n(rap_year, 25, replace = T)

rapstrap

rapstrap_data <- rap_year %>%
  summarize(prop = mean(year))
...
```{r}
score_result <- select(pl_data, score_diff)

score_result <- score_result %>% mutate(h_win = case_when(score_diff > 0 ~ 1,
                                          score_diff <= 0 ~ 0))
h_result <- select(score_result, h_win)

h_win_prop <- summarise(h_result, prop_h_win = mean(h_win))
...
sp <- as_data_frame(sampled_proportions)
...
No it is significantly lower than what the actual data suggest. The peak appears around .33 as expected if all 3 outcomes are equally likely but the actual proportion is around .41 which has very little/no representation in these samples.
...
Null: There is no advantage for the home team in terms of winning games 
Alt: The home team has advantage in games
...
The likelihood the actual proportion from the real data would be returned in the case where all 3 outcomes are equally likely.  
