...
The majority of the estimates are positive implying that generally the wage gap is decreasing
...
-306.718 + 2015*.192 + 3.326
""
"Service"
-306.718 + 2016*.192 + 6.077
```
...
```{r}
gender_employment%>% 
  ggplot(aes(x = year, y = wage_percent_of_male, facet = major_category)) +
...
No, this grpah shows that new slopes have emerged which better represnet each category. If the graph showed slopes that were failry similar the parallel trends assumption would be more valid. 
...
```{r}
"Computer, Engineering, Science" 
-1370.472 + 2016*.720 + 1002.853 + 2016*-.495
""
"Service"
-1370.472 + 2016*.720 + 2137.650 + 2016*-1.058
```
The estimate for "Service" for 2016 lowered compared the estimate made with the previous model
...
A parallel trend model is more simplistic and can be used to more rapidly show general trends across data or make estimates on the entire set with the added consequence of less accuracy.
...
These varibales all show a low level of correlation meaning that these variables are poor predicters of changes in the other variables selected.
...
The positive and relatively small value for "percent_female" implies that the more women in the field slightly decreases the wage gap. It is expected that it would have a small effect due to more entries for female and male wages could slightly shift around their respective averages however it is difficult to say concretely that a large change would've occurred. 
...
R squared is a measure of the difference of each entry of data to the model that is fit to that data squared. This highlights the variance of the entries and how well the model fits the data.
...
The R squared for the multiple_fit model is significantly higher than that of the simple_fit model. This implies that the adding the variable "percent_female", increases the accuracy of the model.
